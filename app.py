...
# 🔥 RE-CENTERED BACKGROUND CLOCK PIPELINE (FIXED INTERSPACE SPACING OVERLAP MARGINS)
while True:
    clock = datetime.now(local_tz).strftime("%Y-%m-%d %I:%M:%S %p")
    clock_placeholder.markdown(f"<p style='text-align:center; color:#00ffcc; font-family:monospace; margin-top:-15px; margin-bottom:15px; font-weight:600; font-size:14px; letter-spacing:1px;'>⚡ SYSTEM STATUS: ACTIVE | MATRIX LIVE SYNC TIME: {clock}</p>", unsafe_allow_html=True)
    time.sleep(1)
