...
def get_ai_unscripted_card_analysis(ticker, price, target_p, pct, sig, api_key):
    ...
    prompt = f"Analyze asset data and return a JSON object with keys 'strat' and 'intel'. Asset: {ticker}, Price: CAD ${price:,.2f}, Target: CAD ${target_p:,.2f}, Shift: {pct:+.2f}%, Signal: {sig}. 'strat' must be a 1-2 sentence real-time portfolio log entry detailing what you (the AI tracker) are doing for yourself right now so copy-traders can replicate the move. Prefix with '📈 **AI TRADING LOG**: ' or '📉 **AI TRADING LOG**: '. 'intel' must be a data volume intelligence overview of option/whale books. Prefix with '📰 **Live Market Intelligence**: '. Institutional tone only, do not mention customers."
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            parsed = json.loads(json.loads(response.read().decode("utf-8"))["choices"]["message"]["content"])
            ...
            if strat_val and intel_val: return str(strat_val), str(intel_val)
            raise ValueError("Parse Key Error")
