...
# 1. PREMIUM HEADER & VISUAL LAYOUT MATRIX
...
# 2. RENDER THE PRIMARY CORE BRAND HEADER LAYOUT WITH EMBEDDED TIME PASSED
...
if submit_button and user_input_text:
    ...
    with st.chat_message("assistant"):
        ...
        if api_key_target == "WIPE": ai_reply = f"Live feed status: {ctx_data} Setup your Groq Key inside your secrets box to unleash unscripted deep learning conversations!"
            ...
            try:
                url = "https://groq.com"
                req = urllib.request.Request(url, data=json.dumps({"model": "openai/gpt-oss-120b", "messages": [{"role": "system", "content": f"You are an expert unscripted financial analyst brain. Have intelligent conversations. Live data: {ctx_data}"}, {"role": "user", "content": user_input_text}]}).encode("utf-8"), headers={"Authorization": f"Bearer {api_key_target}", "Content-Type": "application/json"}, method="POST")
                with urllib.request.urlopen(req) as response:
                    res_d = json.loads(response.read().decode("utf-8"))
                    ai_reply = res_d["choices"]["message"]["content"] if "choices" in res_d and isinstance(res_d["choices"], list) else res_d["message"]["content"]
...
# ⏱️ NATIVE BACKGROUND REFRESH INTERVAL AUTOMATOR
time.sleep(15)
st.rerun()
