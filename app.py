...
st.markdown("""
    ...
    .metric-box::before { content: ''; position: absolute; top: -2px; bottom: -2px; left: -2px; right: -2px; z-index: -1; border-radius: 14px; animation: border-glow-wave 4s linear infinite; }
    .glow-hold::before { background: linear-gradient(90deg, #ffcc00, #b59500, #ffea00, #ffcc00); background-size: 300% 300%; }
    .glow-buy::before { background: linear-gradient(90deg, #00ffcc, #00b591, #6366f1, #00ffcc); background-size: 300% 300%; }
    .glow-sell::before { background: linear-gradient(90deg, #ff4b4b, #b52a2a, #ff7676, #ff4b4b); background-size: 300% 300%; }
    @keyframes border-glow-wave { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
    </style>
...
# 3. INTERACTIVE CHAT ENGINE WITH LIGHTWEIGHT FLAT RE-ROUTING
st.markdown("---")
...
if submit_button and user_input_text:
    ...
    with st.chat_message("assistant"):
        ...
        # FIXED FLAT ROUTING PROTOCOL - INSTANT 120B ENGINE HANDSHAKE
