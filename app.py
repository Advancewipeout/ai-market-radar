...
with st.spinner("📥 Synchronizing core market pricing vectors..."):
    for index, (ticker, display_name) in enumerate(watchlist.items()):
        try:
            ...
            signal, color = ("🟡 HOLD / WAIT FOR CONFIRMATION", "#ffcc00") if abs(pct) <= 0.5 else (("🟢 STRONG BUY / ENTER LONG", "#00ffcc") if pct > 0.5 else ("🔴 STRONG SELL / ENTER SHORT", "#ff4b4b"))
            ...
            with col1 if index % 2 == 0 else col2:
                st.markdown(f"<div class='metric-box' style='border-left-color:{color};'><h2 class='asset-header'>{display_name}</h2><hr style='border-color:#222b3c;'><p style='font-size:16px;'><b>Current Market Price:</b> CAD ${price:,.2f}</p><p style='font-size:18px;'><b>SYSTEM ACTION:</b> <span style='color:{color}; font-weight:bold;'>{signal}</span></p><div class='ai-analysis'>🤖 <b>Neural AI Analyst:</b> {txt}</div></div>", unsafe_allow_html=True)
    ...
    with st.chat_message("assistant"):
        ...
        if api_key_target == "WIPE":
            ...
            try:
                ...
                completion = client.chat.completions.create(model="llama-3.3-70b-versatiles", messages=[{"role": "system", "content": f"You are a premium financial analyst assistant. Answer questions naturally, beautifully, and creatively. Real-time market context values: {ctx_data}. Limit response to 2 short sentences."}, {"role": "user", "content": user_input_text}])
