...
# ==============================================================================
# 2. CORE MARKET DATA CAPTURE AND MULTI-ASSET AI PIPELINE
# ==============================================================================
watchlist = {
...
def get_ai_unscripted_card_analysis(ticker, price, target_p, pct, sig, api_key):
    ...
    # Clean text wrapping to eliminate any inner quote compilation blocks completely
    prompt_instruct = "Your output MUST be a JSON object with exactly two keys: strat and intel."
    prompt_body = f"Asset: {ticker}, Price: CAD ${price:,.2f}, Target: CAD ${target_p:,.2f}, Shift: {pct:+.2f}%, Signal: {sig}."
    prompt_final = f"Analyze this data. {prompt_instruct} 'strat' must be a real-time portfolio trade log entry detailing what you (the AI tracker) are doing for yourself right now so copy-traders can replicate it. Prefix with '📈 **AI TRADING LOG**: ' or '📉 **AI TRADING LOG**: '. 'intel' must be a data volume overview of options, books, or whales. Prefix with '📰 **Live Market Intelligence**: '. Institutional tone only. {prompt_body}"
