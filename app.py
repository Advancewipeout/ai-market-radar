...
# 1. PREMIUM BRANDING & ORIGINAL STYLE MATRICES (WITH THIN BORDER FIX)
...
# 2. COMPACT NATIVE HTTP BUNDLED NET PASS (NO LOG LUGS)
api_key_target = st.secrets.get("GROQ_API_KEY", "WIPE")
ai_strat_list, ai_intel_list = [], []
if api_key_target != "WIPE" and market_summary_list:
    try:
        url = "https://groq.com"
        master_prompt = f"Act as an institutional portfolio engine. For each asset, generate TWO text strings: 1) a clear holding sentence explaining exactly how long the model intends to hold the asset and at what price target it will execute a sell order, 2) a separate brief market sentiment summary for that coin right now. Speak directly to platform users. Return output strictly as valid raw JSON matching this schema: {{\"strategy_sentences\": [\"strat 1\", \"strat 2\", \"strat 3\", \"strat 4\", \"strat 5\", \"strat 6\"], \"intelligence_sentences\": [\"intel 1\", \"intel 2\", \"intel 3\", \"intel 4\", \"intel 5\", \"intel 6\"]}}. Keep array items in order. Market data: {', '.join(market_summary_list)}"
        ...
        with urllib.request.urlopen(req) as r:
            ai_data = json.loads(json.loads(r.read().decode("utf-8"))["choices"][0]["message"]["content"])
            ai_strat_list, ai_intel_list = ai_data.get("strategy_sentences", []), ai_data.get("intelligence_sentences", [])
    except: pass
