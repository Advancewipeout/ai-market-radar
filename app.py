...
# 1. PREMIUM HEADER CONFIG & PERFECT 2PX CHASING-TAIL NEON BORDER LAYOUT CORE ENGINE
...
def get_ai_unscripted_card_analysis(ticker, price, target_p, pct, sig, api_key):
    ...
    try:
        ...
        prompt = f"Act as an independent quantitative trading AI. Analyze asset data and return a JSON object with keys 'strat' and 'intel'. Asset: {ticker}, Price: CAD ${price:,.2f}, Target: CAD ${target_p:,.2f}, Shift: {pct:+.2f}%, Signal: {sig}. Your previous choices: [{history_context}]. 'strat' must be a 1-2 sentence real-time portfolio entry log detailing your internal thinking, explicit system choices (Buy, Close, Scale, Hold), and provide a definitive timeline estimation for human viewers to maintain. 'intel' must be a 1-sentence data volume intelligence overview profiling blocks or whale actions. Adapt messaging based on memory path to avoid loops. Tone institutional."
        completion = client.chat.completions.create(model="llama3-70b-8192", messages=[{"role": "user", "content": prompt}], response_format={"type": "json_object"})
        res = json.loads(completion.choices.message.content)
        s_val, i_val = res.get("strat"), res.get("intel")
        if s_val and i_val:
            st.session_state.rolling_trade_history[ticker].append(str(s_val))
            return str(s_val), str(i_val)
    except: pass
