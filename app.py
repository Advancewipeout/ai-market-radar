...
# 1. PREMIUM CODES & BRANDING CORE STYLING LAYER (THICK 6PX LEFT BORDERS FULLY RESTORED!)
...
# 2. PRO-TIER CONNECTED ENGINE ROUTING FOR YOUR 120B CHATBOT ENDPOINT
...
if api_key_target != "WIPE" and market_summary_list:
    try:
        ...
        with urllib.request.urlopen(req) as r:
            ...
            raw_ai_text = res_json["choices"]["message"]["content"] if "choices" in res_json else res_json["message"]["content"]
...
for index, ticker in enumerate(watchlist.keys()):
    if ticker in asset_data_store:
        ...
        with col1 if index % 2 == 0 else col2:
            # 🎨 BRIDGED BACK TO PREMIUM THICK LEFT INDICATOR EDGES!
            st.markdown(f"""
            ...
            if api_key_target == "WIPE": ai_reply = f"Live feed status: {ctx_data} Setup your Groq Key to unleash unscripted conversations!"
                ...
                try:
                    ...
                    with urllib.request.urlopen(req) as response:
                        ...
                        ai_reply = res_d["choices"]["message"]["content"] if "choices" in res_d else res_d["message"]["content"]
